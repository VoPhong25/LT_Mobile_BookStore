...
public class ProfileActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (user_id == 0) {
            // Xử lý khi không tìm thấy user_id trong SharedPreferences
            Toast.makeText(this, "User ID not found in SharedPreferences", Toast.LENGTH_SHORT).show();
            // Có thể kết thúc hoặc chuyển hướng đến màn hình khác
            // Ví dụ: finish();
            return;
        }
        ...
        if (users == null) {
            // Xử lý khi không tìm thấy người dùng trong cơ sở dữ liệu với user_id đã lưu
